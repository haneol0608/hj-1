<?php
    ...
    // 도면 관리
    if($_SESSION['page'] == 'sub0101') {
      if($search_type && $search) {
        $sql = "SELECT * FROM hj_draw WHERE $search_type LIKE '%$search%' ";
      } else {
        $sql = "SELECT * FROM hj_draw ";
      }
    }

    if($_SESSION['page'] == 'sub0201') {
      if($search_type && $search) {
        $sql = "SELECT * FROM hj_draw WHERE $search_type LIKE '%$search%' ";
      } else {
        $sql = "SELECT * FROM hj_draw ";
