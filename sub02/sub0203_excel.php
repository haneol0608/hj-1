<?php
  ...
  $list_lot = base64_decode($_GET['list_lot']);
    ...
    if($lot_date != null && $list_lot != null) {
        ...
        FROM hj_lada
        WHERE lot_date = '$lot_date' AND list_lot = '$list_lot' AND qt_no IS NOT NULL AND quantity_ok IS NOT NULL
        GROUP BY ho, por, qt_no, series, revision, sp, lot_date, list_lot 
        ORDER BY qt_no * '1' ASC, qt_no ASC";
    } else {
        $select_query = "SELECT ho, por, qt_no, SUM(count) AS count, SUM(weight) AS weight, SUM(money) AS money, MIN(pro_date) AS pro_date, MIN(mp_date) AS mp_date, series, revision, sp, lot_date, list_lot 
        FROM hj_lada 
  ...
  while($row = mysqli_fetch_assoc($select_result)) {
    ...
    $j++;
    $show_paint = [];
    $select_paint_query = "SELECT paint, COUNT(paint) AS count_paint FROM hj_lada WHERE ho = '$ho[$i]' AND por = '$por[$i]' AND paint != '' AND qt_no = '$qt_no[$i]' GROUP BY ho, por, paint, qt_no ";
  ...
  foreach ($por as $key => $value) {
    ...
    $select_seq_query = "SELECT GROUP_CONCAT(DISTINCT(seq) SEPARATOR ', ') AS seq FROM hj_lada WHERE ho = '$ho[$i]' AND por = '$por[$i]' AND qt_no = '$qt_no[$i]'  ORDER BY seq ASC";
  ...
  foreach ($series as $key => $value) {
    ...
    $select_lap_query = "SELECT COUNT(lap) AS count_lap FROM hj_lada WHERE ho = '$ho[$i]' AND por = '$por[$i]' "; 
    $select_lap_query .= "AND qt_no = '$qt_no[$i]' AND paint != '' AND lap !='' ";
