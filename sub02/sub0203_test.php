          ...
          <div class="draw_table">
            <table>
              ...
              <tbody>
                <?php
                    $i = 0;
                    $select_query = "SELECT list_title, lot_date, list_lot FROM hj_lada WHERE lot_date IS NOT NULL AND list_lot IS NOT NULL GROUP BY lot_date, list_lot, list_title";
              ...
              <tbody>
                <?php
                  ...
                  if($get_lotDate != null && $get_listLot != null) {
                    ...
                    $select_query = "SELECT ho, por, qt_no, SUM(count) AS count, SUM(weight) AS weight, SUM(money) AS money, MIN(pro_date) AS pro_date, MIN(mp_date) AS mp_date, series, revision, sp, lot_date, list_lot
                    FROM hj_lada
                    WHERE lot_date IS NULL AND list_lot IS NULL AND qt_no IS NOT NULL AND DATE_FORMAT(quantity_ok, '%Y-%m-%d') IS NOT NULL
                    GROUP BY ho, por, qt_no, series, revision, sp, lot_date, list_lot ";
                  }
                  //*******************************************************************************************************//
