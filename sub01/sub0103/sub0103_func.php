<?php 
    function lap($conn, $ho, $por, $qt_no, $count) {
        $select_lap_query = "SELECT COUNT(A.lap) AS count_lap FROM hj_cutting AS A RIGHT JOIN hj_prolist AS B ON A.por = B.por AND A.ho = B.ho AND A.seq = B.seq WHERE A.ho = '$ho' AND A.por = '$por' AND B.qt_no = '$qt_no' AND A.paint != '' AND A.lap !=''";
