...
<?php
  ...
  // 1. 배열 화 -> 중복 제거
  $qtNo_arr = array();
  $select_query = "SELECT * FROM hj_prolist AS A, hj_cutting AS B WHERE (A.lot_date = '$lot_date' AND A.list_lot = '$list_lot') AND (B.w = '$w' AND B.p= '$p') AND (A.ho = B.ho AND A.por = B.por AND A.seq = B.seq)
  ORDER BY A.qt_no * '+1', A.qt_no ASC, A.seq ASC, B.count DESC";
      ...
      <table>
        ...
        <tbody>
          <?php
            $a = 0;
            $counter = 1;
            $select_query = "SELECT * FROM hj_prolist AS A, hj_cutting AS B WHERE (A.lot_date = '$lot_date' AND A.list_lot = '$list_lot') AND (B.w = '$w' AND B.p= '$p' AND A.lot_date = '$lot_date' AND A.list_lot = '$list_lot') AND (A.ho = B.ho AND A.por = B.por AND A.seq = B.seq) ORDER BY  A.qt_no * '1' ASC, A.qt_no ASC, A.seq ASC, B.count DESC";
            $select_result = mysqli_query($conn, $select_query);
            while($row = mysqli_fetch_assoc($select_result)) {
              $a++;
              $lap = $row['lap'];
