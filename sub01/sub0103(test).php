          ...
          <div class="draw_table">
            <table>
              ...
              <tbody>
                <?php 
                    $i = 0;
                    $select_query = "SELECT list_title, lot_date, list_lot FROM hj_prolist AS A INNER JOIN hj_quantity AS B WHERE lot_date IS NOT NULL AND list_lot IS NOT NULL GROUP BY lot_date, list_lot, list_title";
                    ...
                    while($row = mysqli_fetch_assoc($select_result)) {
                        ...
                        $select_date_query = "SELECT MIN(pro_date) AS min_proDate, MAX(pro_date) AS max_proDate, MAX(mp_date) AS max_mpDate FROM hj_prolist AS A INNER JOIN hj_quantity AS B ON A.ho = B.ho AND A.por = B.por AND A.qt_no = B.qt_no  WHERE lot_date = '$lot_date' AND list_lot = '$list_lot' GROUP BY lot_date, list_lot";
                        ...
                        // $first_title = $year . "년 " . $month . "월 " . $list_lot . "차 공정현황" . " ($min_proDate ~ $max_mpDate)"
                ?>
                <tr>
                  <td><?php echo $i; ?></td>
                  <td>
                  ...
                  <td>
                    <input type="button" onclick="select_prolist('<?php echo $enc_lotDate; ?>', '<?php echo $enc_listLot; ?>');" value="상세조회">
                    <input type="button" onclick="prolist_print('<?php echo $enc_lotDate; ?>', '<?php echo $enc_listLot; ?>');" value="공정리스트 인쇄">
                    <input type="button" onclick="cut_print('<?php echo $enc_lotDate; ?>', '<?php echo $enc_listLot; ?>');" value="컷팅지 인쇄">
                  </td>
                </tr>
                <?php } ?>
              </tbody>
      ...
      <div class="container sub1">
        <div class="sub_plate">
          ...
          <?php
            ...
            //*******************************************************************************************************//
          ?>
          <input type="button" onclick="sub_excel('<?php echo $get_lotDate; ?>', '<?php echo $get_listLot; ?>');" value="엑셀 출력하기">
            
            <?php if($_COOKIE['HPLEVEL'] == "관리") { ?>
                <input type="button" onclick="other_update2();" value="전체 저장">
                <input type="button" onclick="prolist_delete();" value="선택공정 취소">
            <?php } ?>

          <?php if(!isset($get_lotDate)) { ?>
            <?php if($_COOKIE['HPLEVEL'] == "관리") { ?>
            <input type="button" onclick="insert_proList();" value="공정리스트 확정">
            <?php } ?>
          <?php } ?>
              ...
              <tbody>
                <?php
                  ...
                  if($get_lotDate != null && $get_listLot != null) {
                    ...
                    echo $select_query = "SELECT A.ho AS ho, A.por AS por, A.qt_no AS qt_no, SUM(B.count) AS count, SUM(B.weight) AS weight, SUM(money) AS money, MIN(B.pro_date) AS pro_date, MIN(B.mp_date) AS mp_date, A.series AS series, A.revision AS revision, A.sp AS sp, A.lot_date, A.list_lot 
                  ...
                  include 'sub0103/sub0103_func.php';
                ...
                <tr">
                  ...
                  <td onchange="proDate_up(this, '<?php echo $ho; ?>', '<?php echo $por; ?>', '<?php echo $qt_no; ?>');">
                    <?php if($_COOKIE['HPLEVEL'] == "관리") { ?>
                  ...
                  <td onchange="mpDate_up(this, '<?php echo $ho; ?>', '<?php echo $por; ?>', '<?php echo $qt_no; ?>');">
                    <?php if($_COOKIE['HPLEVEL'] == "관리") { ?>
