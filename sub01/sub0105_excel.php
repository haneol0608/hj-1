<?php
    ...
    $select_query = "SELECT * FROM hj_label WHERE stat = '마킹대기' ORDER BY la_no ASC";
    ...
    $cells = array(
        ...
        'C' => array(30, 'POR', 'POR'),
        'D' => array(20, 'SEQ', 'SEQ'),
        'E' => array(20, 'BLOCK', 'BLOCK'),
        'F' => array(20, '피스번호', '피스번호'),
        'G' => array(20, '페인트코드', '페인트코드'),
        'H' => array(20, 'Lot 번호', 'Lot 번호')
    );
    ...
    $objPHPExcel->getActiveSheet()->setCellValue("F1", "피스번호");
    $objPHPExcel->getActiveSheet()->setCellValue("G1", "페인트코드");
    $objPHPExcel->getActiveSheet()->setCellValue("H1", "페인트코드");

    foreach ($cells as $key => $value) {
        $cellName = $key.'1'; //엑셀의 1행 지정
        $objPHPExcel->getActiveSheet()->getColumnDimension($key)->setWidth($value[0]); // 열 넓이 조절
    }
    $objPHPExcel->getActiveSheet()->getStyle('A1:H1')->applyFromArray($cell_style1);
    ...
    $filename = iconv("UTF-8", "EUC-KR", "라벨 마킹대기_".date("Y-m-d"));
